...
function sendMail($numEmail=""){
    ...
    */
    ...
    // Incluir Attach.                    
    $mail->AddAttachment("../edi_files/edi/entradasMX.edi","entradasMX.edi");
    ...
    */
    ...
    // ---------------------------------------------------------        
    unlink("../edi_files/csv/entradasMX.csv");
    //unlink("../edi_files/csv/salidas.csv");
    unlink("../edi_files/edi/entradasMX.edi");
...
function showForm($form="",$error=""){
    ...
    $t->set_file("page", "mxEdiE.inc.html");
...
function getEdiMSC($deli,$ori,$des,$tipoMov){        
    ...
    // Copia el archivo del usuario al directorio ../files
    if ( is_uploaded_file($HTTP_POST_FILES['userfile']['tmp_name']) ) {
        copy($HTTP_POST_FILES['userfile']['tmp_name'], "../edi_files/csv/entradasMX.csv");
    }
    else {
    ...
    // -------------------------------------------------
    $fp = fopen("../edi_files/csv/entradasMX.csv","r");
    $l=1;            
    unset($data);
    unset($dataX);
    while ( $data = fgetcsv($fp,1000,$deli) ) {
    ...
    if( $validFile=="True" ){                    
        ...
        $f=date(Ymd);            
        //$fileEDI = "../edi_files/edi/$fileName";                
        $fileEDI = "../edi_files/edi/entradasMX.edi";
        $fp2 = fopen("$fileEDI","w");
        $sepa="\n";                
        // Terraports tiene el sig, codigo segun MSCA.
        $codePatio = "MEX03";
        ...
        /*
        Este EDI no lleva codigo de destino por que son Entradas.
        switch($des){                        
            case "VER":
            $codePatioD = "VER00";
            break;
        ...
        while ($data = fgetcsv($fp,1000,"$deli")) {
            if( $l > 3 ){
                ...
                if( !empty($conte) && ($flgAnoOk==1)  ){
                    ...
                    //---------------------------------------------------------
                    //COMO SABESMOS SI ES 2 EXPORT ó 3 IMPORT + 4 EMPTY Ó 5 FULL
                    // TipoMov : Este define si el contenedor esta lleno o vacio.                    
                    //----------------------------------------------------------                 
                    // FTX : Remarks +AAI = General Information                 
