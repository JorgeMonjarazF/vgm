...
function sendMail($numEmail=""){
    ...
    */
    ...
    $mail->AddAttachment("../edi_files/edi/salidasMX.edi","salidasMX.edi");
    ...
    */
    ...
    unlink("../edi_files/csv/salidasMX.csv");
    //unlink("../edi_files/edi/entradas.edi");
    unlink("../edi_files/edi/salidasMX.edi");      
...
function showForm($form="",$error=""){
    ...
    $t->set_file("page", "mxEdiS.inc.html");
...
function getEdiMSC($deli,$ori,$des,$tipoMov){        
    ...
    if ( is_uploaded_file($HTTP_POST_FILES['userfile']['tmp_name']) ) {
        copy($HTTP_POST_FILES['userfile']['tmp_name'], "../edi_files/csv/salidasMX.csv");
    }
    else {
    ...
    unset($data);
    unset($dataX);
    $fp = fopen("../edi_files/csv/salidasMX.csv","r");
    $validFile="True";
    ...
    if( $validFile=="True" ){                    
        ...
        $fileEDI = "../edi_files/edi/salidasMX.edi";
        $fp2 = fopen("$fileEDI","w");
        $sepa="\n";                
        ...
        while ($data = fgetcsv($fp,1000,"$deli")) {
            if( $l > 3 ){
                ...
                if( empty($bkgNumber) ){
                    $msgErr[] = "<font color=red>[Error][$conte] Booking no especificado. </font>";                                       
                }
                if( empty($conte) ){
                    $msgErr[] = "<font color=red>[Error] Contenedor no especificado. </font>";                                       
                }
                // -------------------                

                if( count($msgErr)==0 || empty($msgErr)  ){
                    $tlConte++;     
                    $tamano= $dataX[2];                    
                    $cliente = $dataX[13];
                    ...
                    //---------------------------------------------------------
                    // 2 EXPORT ó 3 IMPORT // 4 EMPTY Ó 5 FULL
                    //----------------------------------------------------------
                    // FTX : Remarks +AAI = General Information
                    $bkgNumber=""; // Vacio por ser posicionamiento.
                    ($tipoMov=="E")?$tmov=4:$tmov=5;                    
                    $enc2= "EQD+CN+$conte+$tipo++2+$tmov'".$sepa;
                    $tlSegmentos++;                
                    ...
                    $enc2.="LOC+99+$codePatioD'".$sepa;   // 99 : Place of empty equipment return
                    $tlSegmentos++;                
                    fputs($fp2,$enc2);
                }
            }
...
switch($modo){
    case "validar":
        //almacenarCSV($deli);        
        showForm();
        getEdiMSC($deli,$ori,$des,$tipoMov);        
        break;
    case "email":        
        sendMail();
        showForm();
