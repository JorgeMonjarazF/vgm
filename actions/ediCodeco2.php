...
if( isset($sesIdUsuario) ) {
    ...
    function showForm($form="",$msg=""){
        ...
        if( $sesIdUsuario==10 ){
            ...
            $t->set_file("page", "ediCodecoTrane.inc.html");
        }
        elseif ($sesIdUsuario==19) {
            // Anakosta
            $t->set_file("page", "ediCodecoAnakosta.inc.html");   
        }
        elseif ($sesIdUsuario==17) {
            // Alpasa
            $t->set_file("page", "ediCodecoAlpasa.inc.html");   
        }
        elseif( $sesIdUsuario==14 ){
            // MXGYM: Guaymas
            $t->set_file("page", "ediCodeco2gym.inc.html");

            // Combo de codigos de todos los patios
            $sql="
            SELECT *
            FROM CODIGOS
            order by rs
            ";
            $db->query($sql);
            $t->set_block("page","blqCodPatio","lnCode");
            while( $db->next_record() ){
                $codPatio = $db->f(cod_patio);
                $rs = $db->f(rs);
                $t->set_var(array(
                    "CODPATIO"=>$codPatio,
                    "RS"=>$rs,
                ));
                $t->parse("lnCode","blqCodPatio",true);
            }
        }
        else{
            $t->set_file("page", "ediCodeco2.inc.html");
        }
        ...
        if( $pais=="MX" ){
            ...
            $t->set_var("COD_DES","LMR00");$t->set_var("COD_NAME","ALTAMIRA");$t->parse("lnCodDes","blqCodDes",true);
            $t->set_var("COD_DES","LZC00");$t->set_var("COD_NAME","LAZARO CARDENAS");$t->parse("lnCodDes","blqCodDes",true);
            $t->set_var("COD_DES","ZLO00");$t->set_var("COD_NAME","MANZANILLO");$t->parse("lnCodDes","blqCodDes",true);
            $t->set_var("COD_DES","MEX00");$t->set_var("COD_NAME","MEXICO");$t->parse("lnCodDes","blqCodDes",true);
            $t->set_var("COD_DES","VER00");$t->set_var("COD_NAME","VERACRUZ");$t->parse("lnCodDes","blqCodDes",true);
        }
        elseif( $pais=="SV" ){
            $t->set_block("page","blqCodDes",lnCodDes);
            $t->set_var("COD_DES","AQJ00");$t->set_var("COD_NAME","ACAJUTLA");$t->parse("lnCodDes","blqCodDes",true);
            $t->set_var("COD_DES","GTP01");$t->set_var("COD_NAME","PUERTO BARRIOS");$t->parse("lnCodDes","blqCodDes",true);
        }
        */
